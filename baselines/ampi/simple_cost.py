...
def build_act_cost(make_obs_ph, q_func, num_actions, scope="ampi", reuse=None):
...
def learn_cost(env,
    ...
    act = build_act_cost(make_obs_ph, q_func, num_actions=env.action_space.n, scope=scope, reuse=reuse)
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        z_j = tf.one_hot(tf.argmax(q_tp1, axis=1), num_actions) # max Q(s',a')

        # weight computation
        q_min = tf.reduce_min(q_tp1, axis=1, keep_dims=True)
        q_max = tf.reduce_max(q_tp1, axis=1, keep_dims=True)
        w_pi = (q_tp1 - q_min)/(q_max - q_min)

        # classification loss
        cl_error = tf.nn.weighted_cross_entropy_with_logits(
                      logits=pi, targets=z_j, pos_weight=w_pi)
