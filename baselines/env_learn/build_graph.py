...
def scope_name():
    """Returns the name of current scope as a string, e.g. env_learn/q_func"""
...
def build_act(make_obs_ph, q_func, num_actions, scope="env_learn", reuse=None):
...
def build_act_with_param_noise(make_obs_ph, q_func, num_actions, scope="env_learn", reuse=None, param_noise_filter_func=None):
...
def build_train(make_obs_ph, q_func,env_func, num_actions, optimizer, grad_norm_clipping=None, gamma=1.0,
    double_q=True, scope="env_learn", reuse=None, param_noise=False, param_noise_filter_func=None):
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        state_act = tf.concat([obs_t_input.get(),tf.cast(tf.expand_dims(act_t_ph,1),tf.float32)],1) #[s,a]

        #state_act_out = tf.concat([obs_tp1_input.get(),tf.expand_dims(rew_t_ph,1),tf.expand_dims(done_mask_ph,1)],1) #[s',r,done]
        state_act_out = tf.concat([obs_tp1_input.get(),tf.expand_dims(rew_t_ph,1)],1) #[s',r]
