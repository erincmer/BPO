...
def _mlp_to_dist(hiddens, nbins, inpt, num_actions, scope, reuse=False, layer_norm=False):
    with tf.variable_scope(scope, reuse=reuse):
        dists = []
        out = inpt
        for hidden in hiddens:
            ...
            out = tf.nn.relu(out)
        # Q values
        q_out = layers.fully_connected(out, num_outputs=num_actions*nbins, activation_fn=None)
...
def mlp_to_dist(nbins, hiddens=[], layer_norm=False):
    """This model takes as input an observation and returns values of all actions.
    ...
    q_func: function
        q_function for DQN algorithm.
    """
    return lambda *args, **kwargs: _mlp_to_dist(hiddens, nbins, layer_norm=layer_norm, *args, **kwargs)
